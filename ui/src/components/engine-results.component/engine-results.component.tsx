...
class EngineResultsComponent extends React.Component<IEngineInput, any> {
    getDisplayBoardThickness(roughLumberThickness: RoughLumberThicknessEnum): string {
    ...
    render() {
        return (
            <Connect query={graphqlOperation(engineQuery, this.props)}>
                {
                    ({ data, loading, error }: { data: IEngineResponse, loading: boolean, error: Array<Error> }) => {
                        ...
                        return <div>
                            ...
                            <h3>Board Thickness: {this.getDisplayBoardThickness(data.cuttingBoardPlan.roughLumberThickness)}</h3>
                            <h3>Build Process</h3>
                            <ol>
